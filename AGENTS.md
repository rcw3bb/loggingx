...
- `.github/workflows/tests.yml` — Tests: lint (pylint 10/10) then test with ≥90% coverage gate
...
- Follow SOLID: one responsibility per class, depend on abstractions not concretions.Testsersion
...
- Keep version numbers consistent across `AGENTS.md`, `pyproject.toml`, `README.md`, and `logenrich/__init__.py` (`__version__`). Update all four together on every version bump.
